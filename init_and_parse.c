...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/16 19:22:51 by dmylonas          #+#    #+#             */
/*   Updated: 2021/10/17 14:16:16 by dmylonas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_args(int argc, char **argv, t_p *p)
{
	if ((argc == 5 || argc == 6) && is_numeric(argv, 0, 1))
	{
		...
		p->a.t_to_die = ft_atoi(argv[2]);
		p->a.t_to_eat = ft_atoi(argv[3]);
		p->a.t_to_sleep = ft_atoi(argv[4]);
		p->a.m_eat = -1;
		if (argc == 6)
			p->a.m_eat = ft_atoi(argv[5]);
		if (p->a.total <= 0 || p->a.t_to_die <= 0 || p->a.t_to_eat <= 0
			|| p->a.t_to_sleep <= 0)
			return (0);
		return (1);
	}
